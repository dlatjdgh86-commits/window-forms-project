...
namespace KwuTodoAI
{
    public class TodoItem
    {
        ...
        // 마감기한에 따른 색상: 1일전=빨강, 3일전=노랑, 7일전=초록, 그 이상=회색
        public Color DeadlineColor => DDay switch
        {
            <= 1  => Color.FromArgb(220, 60,  60),   // 빨강 (D-1 이하)
            <= 3  => Color.FromArgb(220, 160, 20),   // 노랑 (D-3 이하)
            <= 7  => Color.FromArgb(50,  170, 90),   // 초록 (D-7 이하)
            _     => Color.FromArgb(130, 140, 155),  // 회색 (일주일 이상)
        };
    ...
    public class AcademicEvent
    {
        ...
        public string   Category { get; set; } = ""; // 시험, 행정, 공휴일 등
